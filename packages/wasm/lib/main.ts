...
export class VGG<T extends string> {
  ...
  constructor(props: VGGProps) {
    ...
    if (props.onStateChange)
      this.on(EventType.StateChange, props.onStateChange);
...

    {
      ...
      if (!isRuntimeScriptExist) {
        try {
          ...
          // check if the local runtime is available
          try {
            // get the local runtime url
            const localRuntimeUrl = localStorage.getItem(
              "local_vgg_runtime_url"
            );
            if (localRuntimeUrl) {
              this.runtime = localRuntimeUrl.replace("/vgg_runtime.js", "");
              await this.loadScript(localRuntimeUrl);
            }
          } catch (err) {
            console.error(err);
      ...
      if (wasmInstance) {
        ...
        if (this.editMode) {
          ...
          Object.assign(globalVggInstances, {
            [this.vggInstanceKey]: {
              instance: wasmInstance,
              listeners: new Map<string, (event: unknown) => void>(), // Here we store the client's listeners, which will be mapped to the uniqueId and consumed in wasmInstance.
              listener: (event: string) => {
                try {
                  const parsedEvent = JSON.parse(event);
